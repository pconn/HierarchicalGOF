...
Lambda=exp(as.vector(3+Eta_true+rnorm(n_Y)))
Y=rpois(n_Y,Lambda)
hist(Y)

#2) jags model
poisson.spat.model <- function(){
  for (i in 1:n_Y) {
    Y[i] ~ dpois(Lambda[i])
    Lambda[i] <- exp(Mu[i]+Z[i])
    Mu[i] <- beta0 + Eta_y[i]
    Z[i] ~ dnorm(0,tau_iid)
  }
...
jags_params = c("beta0","Lambda","Z","tau","tau_iid","theta","Eta_k")
jags_save =c("beta0","tau","tau_iid","theta")
jags.inits = function(){
  list("beta0"=rnorm(1),"tau"=runif(1,1,10),"tau_iid"=runif(1,1,10),"theta"=round(runif(1,0.5,max_range+0.5)),"Eta_k"=rnorm(n_k),"Z"=rnorm(n_Y))
}
jags_fit = jags(data=jags_data,
                inits=jags.inits,
                jags_params,n.iter=10000,
                model.file=poisson.spat.model)
